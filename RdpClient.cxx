...
public:
	...
	virtual int Setup(std::string host, std::string username, std::string password)
	{
		...
		freerdp_settings_set_string(settings, FreeRDP_Username, usernamer);
...
private:
	...
	void OnChannelConnectedEventHandler(void *context, const ChannelConnectedEventArgs *e)
	{
	...
	void OnChannelDisconnectedEventHandler(void *context, const ChannelDisconnectedEventArgs *e)
	{
	...
	BOOL freerdp_client_load_static_channel_addin(rdpChannels *channels, rdpSettings *settings,
	...
	void set_default_callbacks(freerdp *instance)
	{
		WINPR_ASSERT(instance);
	...
	BOOL play_sound(rdpContext *context, const PLAY_SOUND_UPDATE *play_sound)
	{
		/* TODO: Implement */
	...
	BOOL keyboard_set_indicators(rdpContext *context, UINT16 led_flags)
	{
	...
	BOOL keyboard_set_ime_status(rdpContext *context, UINT16 imeId, UINT32 imeState,
								 UINT32 imeConvMode)
	{
		if (!context)
			return FALSE;
	...
	int client_start(rdpContext *context)
	{
	...
	int client_stop(rdpContext *context)
	{
